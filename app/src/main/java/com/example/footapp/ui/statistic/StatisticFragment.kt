...
class StatisticFragment : BaseFragment<ActivityStatisticByYearBinding, StatisticViewModel>() {
    ...
    lateinit var spinnerTimeUser: SpinnerAdapter
    ...
    override fun initView() {
        ...
        binding.barchart.setContent {
            ...
            var isShowBarChart by remember {
            ...
            LaunchedEffect(key1 = Unit, block = {
                viewModel.uiState.collect {
                    ...
                    isShowBarChart = it.isShowBarChart
                    binding.tvTotal.text = it.total.formatToPrice()
                }
            })

            BarCharView(stoneValue, barDatas.toMutableList(), isShowBarChart)
        }
    }
...
//        binding.barchart.invalidate()
    ...
    override fun observerLiveData() {
        viewModel.year.observe(viewLifecycleOwner) {
            spinnerAdapter = SpinnerAdapter(it.map { it.toString() }, binding.root.context)
                ...
                viewModel.stateNormal.collect {
                    when (it) {
                        is StatisticViewModel.StateNormal.StaffTime -> {
                            spinnerTimeUser = SpinnerAdapter(it.staffTime, binding.root.context)
